...
const persistentDataPath = api.persistentDataPath();
...
function init(): void {
    ...
    (document.getElementById("customStylesheetLink") as HTMLLinkElement).href = "file:///" + persistentDataPath + "/userStyles.css";
    ...
    /*
    document.getElementById("revertToDefaultDataDirBtnTooltip").title = "Revert to" + persistentDataPath;
...
export function applyPrefsRuntime(needsRestart = false): void {
    ...
    // TODO write prefs to disk, it should do it onClose though
    api.setPrefs(prefs);
...
export function processNotebooks(): void {
    ...
    function draw(item: NotebookItem, container: HTMLElement) {
        ...
        document.getElementById(validatorEscape(item.id)).addEventListener("contextmenu", function (e) {
            ...
            selectedItem = api.getNotebookItem(this.id);
            ...
            else if (selectedItem.type === NBIType.SECTION)
                cm = document.getElementById("section-context-menu");
            else if (selectedItem.type === NBIType.PAGE)
...
document.getElementById("newNotebookBtn").addEventListener("click", () => {
    createNewItemMode = NBIType.NOTEBOOK;
    selectedItem = null;
...
query("#NBCM-newPage").on("click", () => {
    ...
    createNewItemMode = NBIType.PAGE;
});
...
query("#NBCM-newSection").on("click", () => {
    ...
    createNewItemMode = NBIType.SECTION;
});
...
query("#PCM-favoritePage").on("click", () => {
    if (selectedItem.type === NBIType.PAGE) {
